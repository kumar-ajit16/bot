...
def extract_name(text: str):
    match = re.search(r"\b(i am|i'm|this is|my name is)\s+([a-zA-Z]+)", text.lower())
    if match:
...
def get_current_time():
    now = datetime.now()
    return now.strftime("🕒 %I:%M %p on %d %B %Y")
...
def chat(req: ChatRequest):
    ...
    if name:
        return {"reply": f"Nice to meet you {name} 😊 How can I help you today?"}

    if any(w in tokens for w in ["time", "date", "today", "now"]):
        return {"reply": get_current_time()}
    ...
    if any(p in user_message for p in SMALL_TALK):
